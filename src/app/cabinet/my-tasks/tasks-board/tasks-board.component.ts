...
export class TasksBoardComponent implements OnInit {
  ...
  ngOnInit() {
    this._tasksService.refreshBoard$
      .subscribe(_ => {
        this.getTasksList();
      })
  ...
  drop(event: CdkDragDrop<number[]>) {
    ...
    if (previousStatus !== status) {
      this._tasksService.updateObjectiveStatus((event.item?.data as any)?.id, status)
        .pipe(takeUntilDestroyed(this._destroyRef))
        .subscribe({
          next: () => {
  ...
  openDetailsDialog(item: any): void {
      this._matDialog.open(ObjectiveManageModalComponent, {
        width: '700px',
        maxWidth: '100%',
        data: item
      })
  ...
  getTasksList(): void {
    ...
    combineLatest([
      this._tasksService.getObjectivesList(payload1),
      this._tasksService.getObjectivesList(payload2),
      this._tasksService.getObjectivesList(payload3),
    ]).pipe(
